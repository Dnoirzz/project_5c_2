import 'dart:convert';
import 'package:http/http.dart' as http;
import '../models/jurusan_models.dart';

class JurusanService {
  final String baseUrl = "http://192.168.1.5/project_5c_2/backend/";

  Future<List<JurusanModel>> fetchJurusan() async {
    try {
      final response = await http.get(
        Uri.parse("${baseUrl}admin_dashboard.php"),
        headers: {"Authorization": "Bearer mysecretkey"},
      );

      if (response.statusCode == 200) {
        final jsonData = json.decode(response.body);
        final List<dynamic> data = jsonData["data"]["jurusan"];
        return data.map((e) => JurusanModel.fromJson(e)).toList();
      } else {
        throw Exception("Failed to load jurusan data: ${response.statusCode}");
      }
    } catch (e) {
      throw Exception("Error fetching jurusan data: $e");
    }
  }
}
